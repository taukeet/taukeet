...
final getPrayerTimesUseCaseProvider = Provider<GetPrayerTimes>((ref) {
  throw UnimplementedError('getPrayerTimesUseCaseProvider must be overridden with dependency injection');
});

// Provider for current prayer use case
final getCurrentPrayerUseCaseProvider = Provider<GetCurrentPrayer>((ref) {
  throw UnimplementedError('getCurrentPrayerUseCaseProvider must be overridden with dependency injection');
});

// Provider for prayer times
final prayerTimesProvider = FutureProvider.family<List<PrayerTime>, DateTime>((ref, dateTime) async {
...
final currentPrayerProvider = FutureProvider.family<PrayerTime?, DateTime>((ref, dateTime) async {
